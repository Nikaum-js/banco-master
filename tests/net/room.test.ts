...
const host = { token: 'h', name: 'Nik', color: SEAT_COLORS[0] }
...
describe('sala e identidade', () => {
  ...
  it('cor é única por sala; duplicata é recusada (§12.5)', () => {
    const room = createRoom('r1', host)
    const dup = joinRoom(room, { token: 'g', name: 'Amigo', color: SEAT_COLORS[0] })
    expect(dup).toEqual({ ok: false, reason: 'color-taken' })
    const ok = joinRoom(room, { token: 'g', name: 'Amigo', color: SEAT_COLORS[1] })
    expect(ok.ok).toBe(true)
  })

  it('nome duplicado é permitido (Clarifications)', () => {
    const room = createRoom('r1', host)
    const same = joinRoom(room, { token: 'g', name: 'Nik', color: SEAT_COLORS[1] })
  ...
  it('sala cheia (8) recusa o 9º (§11.1)', () => {
    ...
    for (let i = 1; i < MAX_SEATS; i++) {
      const r = joinRoom(room, { token: `g${i}`, name: `G${i}`, color: SEAT_COLORS[i] })
      expect(r.ok).toBe(true)
      if (r.ok) room = r.room
    }
    expect(room.seats).toHaveLength(MAX_SEATS)
    const ninth = joinRoom(room, { token: 'g9', name: 'G9', color: SEAT_COLORS[0] })
    expect(ninth).toEqual({ ok: false, reason: 'room-full' })
  })

  it('após o início, token DESCONHECIDO é recusado (FR-005, §11.2)', () => {
    let room = createRoom('r1', host)
    const g = joinRoom(room, { token: 'g', name: 'G', color: SEAT_COLORS[1] })
    if (g.ok) room = g.room
    const started = startGame(room)
    expect(started.ok).toBe(true)
    if (started.ok) room = started.room
    const stranger = joinRoom(room, { token: 'x', name: 'X', color: SEAT_COLORS[2] })
    expect(stranger).toEqual({ ok: false, reason: 'already-started' })
  })

  it('token JÁ assentado re-anexa ao mesmo assento, antes e depois do início (FR-004)', () => {
    let room = createRoom('r1', host)
    const g = joinRoom(room, { token: 'g', name: 'G', color: SEAT_COLORS[1] })
    if (g.ok) room = g.room
    const started = startGame(room)
    ...
    expect(unknown).toEqual({ ok: false, reason: 'unknown-token' })
  })
  ...
  it('start exige 2+ jogadores; ordem de turno = ordem de entrada (FR-006)', () => {
    ...
    const g = joinRoom(solo, { token: 'g', name: 'G', color: SEAT_COLORS[1] })
