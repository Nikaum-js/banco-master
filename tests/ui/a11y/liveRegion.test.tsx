...
function fakeRoom(): Room {
  return {
    ...
    status: 'playing',
    seats: [
      { playerId: 'p1', uid: 'tok1', name: 'Ana', color: '#d9a650', isHost: true, connected: true, reentryCode: 'AAA111' },
      { playerId: 'p2', uid: 'tok2', name: 'Bia', color: '#a76bf5', isHost: false, connected: true, reentryCode: 'BBB222' },
    ],
  }
}
...
describe('LiveRegion (T028)', () => {
  ...
  it('início da minha vez (ou de uma decisão minha) é anunciado no canal assertivo', () => {
    ...
    useGameStore.setState({ game: g })
    useRoomStore.setState({ room: fakeRoom(), myUid: 'tok2' }) // eu sou p2
