// Contratos de URL da 051: a raiz virou landing e o app mora em `/jogar`, mas nenhum
...
describe('roomLink (FR-005)', () => {
  it('gera convite novo apontando para /jogar', () => {
    expect(roomLink('abc123')).toBe('/jogar?room=abc123')
    expect(roomLink('abc123', 'https://exemplo.test')).toBe('https://exemplo.test/jogar?room=abc123')
  })

  it('escapa o id na query', () => {
    expect(roomLink('a b&c')).toBe('/jogar?room=a%20b%26c')
  })
})

describe('extractRoomId aceita convite colado inteiro', () => {
  it('lê o formato novo (/jogar?room=)', () => {
    expect(extractRoomId('https://exemplo.test/jogar?room=xyz789')).toBe('xyz789')
  })
...
describe('vercel.json traduz os contratos antigos na borda (FR-004)', () => {
  ...
  it.each(['room', 'host', 'local', 'players'])('redireciona /?%s= para /jogar', (param) => {
    ...
    expect(redirect?.destination).toBe('/jogar')
