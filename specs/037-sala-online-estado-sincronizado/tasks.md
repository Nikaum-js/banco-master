...
- [x] **T028** Migration aplicada no projeto `edppdqrkqljhjkbyjvsz` (`rooms`, RLS, Realtime, trigger de `updated_at` com `search_path` fixo). Validada por `scripts/net-smoke.ts` — host + convidado em conexões Realtime distintas, contra a infra real: assento concedido pela rede em **52ms**, estado inicial propagado em **91ms**, comando difundido em **27ms** (SC-002: alvo <1s), estados convergindo byte a byte e **comando forjado descartado pelo host** (SC-005 agora provado também sobre o transporte real, não só no hub in-memory). Verificado ainda pela REST que o upsert parcial de sala **não** zera `game`/`seq` — premissa do `saveRoom` que só dava para confirmar contra o PostgREST. **SC-006 (custo)** segue por construção: o fluxo normal não toca o Postgres.
