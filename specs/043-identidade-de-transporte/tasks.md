...
## Fase 7 — O que a Fase 6 encontrou (D-038 e os defeitos de infra)
...
- [X] **T050** [D-038] O `reentryCode` vazava e era destruído, por três caminhos com uma raiz só (a autoridade lia por um caminho pensado para não-autoridade e persistia o resultado): `read_snapshot` devolvia `seats` cru a todos; `handleSeatReattached` regravava a prévia redigida; e o anfitrião que dava F5 no lobby remontava a sala a partir de `Client.room()`, que não carrega código nenhum. Fechado por `preserve_seat_codes` na gravação (o código é imutável depois de mintado) + seleção por chave nas duas leituras. ADR [D-038](../../docs/adr/D-038-o-codigo-de-reentrada-e-imutavel-e-a-autoridade-o-le.md); provas em `seat-secrets.test.ts` e `reentry.test.ts`.
