...
Devolve `{ id, status, seats: PublicSeat[] }` — sem `reentryCode` de ninguém, **exceto** o do assento de quem chamou.
...
Recusa quem não tem assento na sala. Para quem tem, devolve por **seleção de chave**:

- jogador → `game` + `secrets->auth.uid()`
- anfitrião → `game` + `secrets` inteiro
