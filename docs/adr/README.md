...
- [D-081](D-081-custo-de-construcao-escala-por-nivel.md) — O **custo de construção escala por nível**: `BUILD_LEVEL_MULT` = 1·1·1,25·1,5·2·2,5·3 sobre o tier do grupo (laranja: 110 → 330). Era flat, e com a escada de 7 níveis o topo virava o melhor negócio do tabuleiro — arranha-céu por $110 acrescentando $286 de aluguel, país completo se pagando em ~1,9 visitas. A entrada (níveis 1–2) e o sweet spot da 3ª casa ficam intactos; hotel/2º hotel/arranha caem para ROI 0,5–1,0x. Venda reembolsa metade do **degrau demolido**; patrimônio e liquidação passam a somar a escada real. Refina D-024, corrige efeito colateral da D-076 (SRS §5.1/§5.2/§5.3 v1.41)
- [D-082](D-082-analytics-auto-hospedado-nas-paginas-de-marketing.md) — A **auditoria de marketing mede código do app, não contagem de scripts**: as páginas de marketing podem carregar **um** `<script src>`, e só do host do Plausible auto-hospedado. O `audit:bundle` checava `scriptRefs.length === 0` como *proxy* de FR-006/SC-003 — bom enquanto não existia script de terceiro legítimo, e reprovando o que o requisito permite assim que existiu. FR-006 e SC-003 seguem intactas (nada de `src/game`, `src/net`, stores, `motion` ou Supabase); qualquer outro script externo continua reprovando. Refina o gate da spec 051
