...
4. [x] **Sessão & Resiliência** (§11.3/§11.4, D-016, princípio VII) — desconexão de **qualquer** jogador → **pausa global** + aviso a todos; reconexão pelo mesmo link recarrega do servidor; **reload sem perda**; **host desconectado pausa indefinidamente, sem transferência** (§16); propriedades do desconectado **não** vão ao banco. Status de desconectados no HUD (§12.3). _(2026-07-26, spec 041/D-033/D-034: pausa com causa e relógio durável; gravação serializada/monotônica com retry e sinal de esgotamento; presença reconciliada ao reassumir autoridade; cliente ressincroniza com espera crescente; conexão própria visível na tela; reentrada por código quando o token se perde.)_
   - ~~**Fora do escopo desta spec** (registrado, não resolvido): o endurecimento de identidade de transporte~~ — **fechado pela spec 043** (2026-07-27): a identidade virou o `uid` da sessão anônima atestada pelo servidor ([D-035](adr/D-035-identidade-de-transporte-atestada-pelo-servidor.md)), o remetente passou a vir do **endereço do tópico** em vez do payload, a linha da sala deixou de ser legível/gravável por qualquer portador da chave pública ([D-036](adr/D-036-acesso-a-sala-autorizado-no-servidor.md)), a mão parou de trafegar para quem não é o dono ([D-037](adr/D-037-estado-por-perspectiva-a-mao-nao-trafega.md)) e o código de reentrada virou imutável ([D-038](adr/D-038-o-codigo-de-reentrada-e-imutavel-e-a-autoridade-o-le.md)). Provado contra o projeto real por `scripts/attack.ts` (seis vetores).
...
> **Implementado (2026-07-27)** — o milestone inteiro é a spec [044](../specs/044-polimento-lancamento/spec.md), apoiada em quatro decisões novas: [D-038](adr/D-038-fim-de-jogo-tem-classificacao-e-resumo.md) (classificação e resumo no fim de jogo), [D-039](adr/D-039-acessibilidade-aa-no-caminho-de-jogo.md) (WCAG 2.2 AA no caminho de jogo, com gate no CI; paisagem é a orientação de jogo), [D-040](adr/D-040-telemetria-minima-anonima.md) (telemetria mínima anônima: contagem no Supabase, exceção no Sentry) e [D-041](adr/D-041-publicacao-em-vercel-com-gate-verde.md) (publicação na Vercel, produção só com gate verde). SRS em v1.9 (§9.5, §12.2, §12.6, §12.7, §12.8). **Código entregue; falta o lançamento em si** (migrations em produção, projeto na Vercel, primeiro deploy) — ver [`RUNBOOK.md`](RUNBOOK.md).
