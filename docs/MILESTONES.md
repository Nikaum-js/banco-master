...
- ❌ **031** — **Leilão de escassez de terrenos** (D-023 / SRS §7.3): **revertida pela [D-059](../docs/adr/D-059-leilao-de-escassez-de-terrenos-revertido.md)** em 2026-07-29. O gatilho por contagem de terrenos livres (`maybeOpenLandAuction`, `freeLots`, `landAuctionArmed`, `LAND_AUCTION_THRESHOLD`, `LAND_TRIGGERING`) saiu do motor. Sobreviveu o que a 039 usa: o evento autônomo `GameState.landAuction`, `economy/landAuction.ts` com `placeLandBid` (§7.2 + **trava de solvência** `committedCash`) e `closeLandAuction`/`closeExpiredLandLots` (líder paga banco+escritura; sem lance fica livre), o cronômetro por lote e a `LandAuctionLayer`.
...
4. [x] **Sessão & Resiliência** (§11.3/§11.4, D-016, princípio VII) — desconexão de **qualquer** jogador → **pausa global** + aviso a todos; reconexão pelo mesmo link recarrega do servidor; **reload sem perda**; **host desconectado pausa indefinidamente, sem transferência** (§16); propriedades do desconectado **não** vão ao banco. Status de desconectados no HUD (§12.3). _(2026-07-26, spec 041/D-033/D-034: pausa com causa e relógio durável; gravação serializada/monotônica com retry e sinal de esgotamento; presença reconciliada ao reassumir autoridade; cliente ressincroniza com espera crescente; conexão própria visível na tela; reentrada por código quando o token se perde.)_
   - ~~**Fora do escopo desta spec** (registrado, não resolvido): o endurecimento de identidade de transporte~~ — **fechado pela spec 043** (2026-07-27): a identidade virou o `uid` da sessão anônima atestada pelo servidor ([D-042](adr/D-042-identidade-de-transporte-atestada-pelo-servidor.md)), o remetente passou a vir do **endereço do tópico** em vez do payload, a linha da sala deixou de ser legível/gravável por qualquer portador da chave pública ([D-036](adr/D-036-acesso-a-sala-autorizado-no-servidor.md)), a mão parou de trafegar para quem não é o dono ([D-037](adr/D-037-estado-por-perspectiva-a-mao-nao-trafega.md)) e o código de reentrada virou imutável ([D-043](adr/D-043-o-codigo-de-reentrada-e-imutavel-e-a-autoridade-o-le.md)). Provado contra o projeto real por `scripts/attack.ts` (seis vetores).
...
> O milestone inteiro é a spec [044](../specs/044-polimento-lancamento/spec.md), apoiada em quatro decisões novas: [D-038](adr/D-038-fim-de-jogo-tem-classificacao-e-resumo.md) (classificação e resumo no fim de jogo), [D-039](adr/D-039-acessibilidade-aa-no-caminho-de-jogo.md) (WCAG 2.2 AA no caminho de jogo, com gate no CI; paisagem é a orientação de jogo), [D-040](adr/D-040-telemetria-minima-anonima.md) (telemetria mínima anônima: contagem no Supabase, exceção no Sentry) e [D-041](adr/D-041-publicacao-em-vercel-com-gate-verde.md) (publicação na Vercel, produção só com gate verde). SRS em v1.9 (§9.5, §12.2, §12.6, §12.7, §12.8). **Lançado em produção** (2026-07-27): as quatro migrations aplicadas, projeto ligado na Vercel com o gate de CI promovendo, teste de fumaça e ensaio de rollback do [`RUNBOOK.md`](RUNBOOK.md) §3/§4 executados. **M4 fechado — v1.0 no ar.**

- [x] Animações — vocabulário único de duração/curva e freio de `prefers-reduced-motion` valendo em toda superfície (hoje: 7 pontos que lembraram)
- [x] Acessibilidade AA no caminho de jogo (foco, teclado, trap de modal, leitor de tela, contraste, alvos) + responsivo tablet/celular **landscape**, com aviso de girar em retrato
- [x] Tela de fim de jogo com classificação completa por ordem inversa de eliminação, patrimônio, propriedades e duração
- [x] Sons — entregue na spec 035 · [x] Telemetria mínima (partidas iniciadas/finalizadas/pausadas/erros), anônima
- [x] Deploy na Vercel + CI de produção (gate verde promove) · **aplicar as migrations pendentes desde a 037** · smoke E2E de partida completa + auditoria de acessibilidade no CI
