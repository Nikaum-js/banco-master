...
// Rótulo legível por efeito (apresentação; o motor só guarda o EffectId).
...
function cardLabel(effect: string): string {
  return CARD_LABEL[effect] ?? effect.replace(/([A-Z])/g, ' $1').replace(/^./, (c) => c.toUpperCase())
}

// Descrição curta por efeito (apresentação na revelação, 025). Fallback no nome.
