...
**A operação de lançamento está concluída** (atualizada em 2026-07-30): as migrations do repo estão aplicadas no projeto de produção — **quais**, e como aplicar as próximas, quem diz é o [`docs/RUNBOOK.md`](./docs/RUNBOOK.md) §1, e este arquivo deliberadamente não repete a contagem (repetida em dois lugares ela só existe para divergir, e divergiu: ficou dizendo "cinco" enquanto o RUNBOOK já dizia seis, e a 0006 passou dois dias fora de produção), os secrets da Vercel (`VERCEL_TOKEN`/`ORG_ID`/`PROJECT_ID`) estão no repo, e o `deploy.yml` promove produção **só** depois do CI fechar verde (D-041) — o auto-deploy nativo em `main` está desligado por `vercel.json`. O [`docs/RUNBOOK.md`](./docs/RUNBOOK.md) continua valendo como procedimento de operação, não como pendência.
